...
(function(){
  function initPersianDates(){
    ...
    document.querySelectorAll(selectors.join(',')).forEach(function(el){
      if (seen.has(el) || el.dataset.persianReady === '1') return;
      seen.add(el);
      el.dataset.persianReady = '1';
      el.type = 'text';
      ...
      el.setAttribute('placeholder','انتخاب تاریخ شمسی');
      el.readOnly = true;
  ...
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initPersianDates);
  else initPersianDates();
  setTimeout(initPersianDates, 500);
})();
</script>
'''
...
def home():
    with open("index.html", "r", encoding="utf-8") as f:
        html = f.read()
    if "persian-datepicker.min.css" not in html:
        html = html.replace("</head>", PERSIAN_DATE_ASSETS + "</head>")
    html = html.replace("</body>", PERSIAN_DATE_SCRIPT + "</body>")
...
def save_patient():
    ...
    if patient:
        ...
        patient = Patient(patient_code=patient_code, data=payload)
...
def list_patients():
    ...
    for patient in patients:
        ...
        result.append({"patient_code": patient.patient_code, "patient_name": data.get("patient_name", ""), "age": data.get("age", ""), "sex": data.get("sex", ""), "diagnosis": data.get("diagnosis", "")})
    return jsonify(result)

@app.route("/api/health", methods=["GET"])
def health():
    return jsonify({"status": "ok", "service": "SS-PORM"})

@app.route("/upload")
def upload_page():
    return render_template("upload.html", patient_code=request.args.get("patient", ""))
...
def upload_file(patient_code):
    ...
    file_date = request.form.get("date", "").strip().translate(str.maketrans("۰۱۲۳۴۵۶۷۸۹", "0123456789"))
    file_type = request.form.get("type", "").strip()
    file_stage = request.form.get("stage", "").strip()
    object_key = f"{patient_code_safe}/{filename}"
    s3.upload_fileobj(file, LIARA_BUCKET_NAME, object_key, ExtraArgs={"ContentType": file.content_type or "application/octet-stream", "Metadata": {"title": file_title, "date": file_date, "type": file_type, "stage": file_stage}})
...
def list_patient_files(patient_code):
    ...
    response = s3.list_objects_v2(Bucket=LIARA_BUCKET_NAME, Prefix=prefix)
    files = []
    for obj in response.get("Contents", []):
        filename = obj["Key"][len(prefix):]
        if filename:
            head = s3.head_object(Bucket=LIARA_BUCKET_NAME, Key=obj["Key"])
            metadata = head.get("Metadata", {})
            files.append({"filename": filename, "title": metadata.get("title", ""), "date": metadata.get("date", ""), "type": metadata.get("type", ""), "stage": metadata.get("stage", "")})
...
def get_patient_file(patient_code, filename):
    ...
    try:
        obj = s3.get_object(Bucket=LIARA_BUCKET_NAME, Key=object_key)
        return Response(obj["Body"].read(), mimetype=obj.get("ContentType", "application/octet-stream"))

...
MODQ_SCRIPT=r'''<style>.modq-summary{background:#eef6ff;border:1px solid #b9d7f5;border-radius:10px;padding:12px;margin-top:10px}.modq-summary input{background:white;font-weight:800;font-size:18px;text-align:center}.modq-open{margin-top:10px;background:#1769aa;color:#fff}</style><script>(function(){function openQ(key){window.open('/modq?key='+encodeURIComponent(key),'modq_'+key,'width=760,height=850,scrollbars=yes')}function install(){document.querySelectorAll('.card').forEach(function(card){var h=card.querySelector('h2');if(!h)return;var title=h.textContent||'';if(title.indexOf('Modified Oswestry')<0&&title.indexOf('MODQ')<0)return;if(card.dataset.modqUnified)return;card.dataset.modqUnified='1';var hidden=card.querySelector('input[name="odi"]')||card.querySelector('input[id="odi_hidden"]');var key=hidden&&hidden.name?hidden.name:'odi';var existing=hidden?hidden.value:'';card.innerHTML='<h2>'+h.textContent+'</h2><div class="modq-summary"><label>امتیاز نهایی MODQ (0–100)</label><input type="number" min="0" max="100" step="1" name="'+key+'" id="modq_final_'+key+'" value="'+existing+'" readonly><button type="button" class="modq-open" data-key="'+key+'">📋 باز کردن پرسشنامه MODQ</button><p class="small">جزئیات ۱۰ آیتم در صفحه اختصاصی MODQ ثبت می‌شود و امتیاز نهایی به‌صورت خودکار به این باکس منتقل می‌شود.</p></div>';card.querySelector('.modq-open').onclick=function(){openQ(this.dataset.key)}});document.querySelectorAll('input[name*="odi"],input[name*="modq"]').forEach(function(inp){if(inp.closest('.modq-summary'))return;var name=inp.name||inp.id;if(!name||inp.dataset.modqConverted)return;inp.dataset.modqConverted='1';var wrap=document.createElement('div');wrap.className='modq-summary';wrap.innerHTML='<label>امتیاز نهایی MODQ (0–100)</label><input type="number" min="0" max="100" step="1" name="'+name+'" id="modq_final_'+name+'" value="'+(inp.value||'')+'" readonly><button type="button" class="modq-open" data-key="'+name+'">📋 باز کردن پرسشنامه MODQ</button>';inp.replaceWith(wrap);wrap.querySelector('button').onclick=function(){openQ(this.dataset.key)}})}window.addEventListener('message',function(e){if(e.origin!==location.origin||!e.data||e.data.type!=='ssporm-modq')return;var x=document.getElementById('modq_final_'+e.data.key);if(x&&e.data.score!==null)x.value=e.data.score});document.readyState==='loading'?document.addEventListener('DOMContentLoaded',install):install();setTimeout(install,700)})();</script>'''
PATIENT_LIST_SCRIPT=r'''<style>#patientList{max-height:260px;overflow-y:auto;border:1px solid #ddd;border-radius:10px;margin-top:8px;background:#fff}#patientList .patient-row{display:grid;grid-template-columns:42px 1fr auto;gap:8px;align-items:center;padding:8px 10px;border-bottom:1px solid #eee;font-size:13px}#patientList .patient-main{min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;cursor:pointer}#patientList .row-no{font-weight:700;text-align:center;background:#eef2f3;border-radius:7px;padding:5px 2px}.delete-patient{background:#b42318;color:#fff;padding:7px 9px;font-size:12px;border-radius:7px}.record-no-box{display:flex;align-items:center;gap:10px;margin:10px 0 14px;padding:10px 12px;background:#eef6ff;border:1px solid #b9d7f5;border-radius:10px}.record-no-value{min-width:58px;text-align:center;font-size:18px;font-weight:800;background:#fff;border:1px solid #9fc4ea;border-radius:8px;padding:7px 12px}</style><script>function escHtml(v){return String(v??'').replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]})}async function refreshRecordNumber(){try{const r=await fetch('/api/patients/next-row');if(!r.ok)return;const d=await r.json(),e=document.getElementById('recordNumberValue');if(e)e.textContent=d.next_row}catch(e){}}function installRecordNumber(){const f=document.getElementById('f');if(!f||document.getElementById('recordNumberBox'))return;const box=document.createElement('div');box.id='recordNumberBox';box.className='record-no-box';box.innerHTML='<b>ردیف پرونده</b><span id="recordNumberValue" class="record-no-value">—</span><span class="small">(اتوماتیک)</span>';f.insertBefore(box,f.firstChild);refreshRecordNumber()}window.listPatients=async function(){const box=document.getElementById('patientList');box.textContent='در حال دریافت لیست بیماران...';try{const r=await fetch('/api/patients'),patients=await r.json();if(!r.ok)throw new Error('HTTP '+r.status);if(!patients.length){box.textContent='بیماری ذخیره نشده است.';return}box.innerHTML=patients.map(p=>`<div class="patient-row"><span class="row-no">${p.row_number}</span><span class="patient-main" data-code="${escHtml(p.patient_code)}" data-row="${p.row_number}" onclick="document.getElementById('f').elements['study_id'].value=this.dataset.code;document.getElementById('recordNumberValue').textContent=this.dataset.row;loadPatient();"><b>${escHtml(p.patient_code)}</b>${p.patient_name?' — '+escHtml(p.patient_name):''}</span><button type="button" class="delete-patient" data-code="${escHtml(p.patient_code)}" onclick="deletePatient(this.dataset.code)">حذف</button></div>`).join('')}catch(e){box.textContent='دریافت لیست بیماران انجام نشد: '+e.message}};window.deletePatient=async function(code){if(!confirm('پرونده '+code+' به‌طور کامل حذف شود؟'))return;if(!confirm('تأیید نهایی حذف؟ این عملیات قابل بازگشت نیست.'))return;const r=await fetch('/api/patients/'+encodeURIComponent(code),{method:'DELETE'}),d=await r.json();if(!r.ok){alert(d.error||'خطا');return}await listPatients();await refreshRecordNumber()};document.addEventListener('DOMContentLoaded',installRecordNumber);</script>'''
SAVE_OVERRIDE_SCRIPT=r'''<script>(function(){function collect(){var f=document.getElementById('f'),o={};for(var el of f.elements){if(!el.name||el.type==='file'||el.type==='button')continue;if(el.type==='checkbox')o[el.name]=el.checked;else if(el.type==='radio'){if(el.checked)o[el.name]=el.value}else o[el.name]=el.value}o.patient_code=(o.study_id||'').trim();var bmi=document.getElementById('bmi'),mfi=document.getElementById('mfi');if(bmi)o.BMI=bmi.textContent;if(mfi)o.mFI5=mfi.textContent;return o}window.savePatient=async function(){var o=collect(),s=document.getElementById('saveStatus'),b=document.getElementById('saveBtn');s.style.display='block';if(!o.patient_code){s.textContent='ابتدا کد بیمار / کد مطالعه را وارد کنید.';return}b.disabled=true;s.textContent='در حال ذخیره...';try{var r=await fetch('/api/patients',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify(o)});var d=await r.json();if(!r.ok)throw new Error(d.error||('HTTP '+r.status));s.textContent='✓ اطلاعات بیمار با موفقیت ذخیره شد.';var rn=document.getElementById('recordNumberValue');if(rn&&d.row_number)rn.textContent=d.row_number}catch(e){s.textContent='ذخیره انجام نشد: '+e.message}finally{b.disabled=false}};window.loadPatientFiles=async function(patientCode){var box=document.getElementById('patientFiles');if(!box)return;try{var r=await fetch('/api/files/'+encodeURIComponent(patientCode),{credentials:'same-origin'}),d=await r.json();if(!r.ok)throw new Error(d.error||('HTTP '+r.status));var files=d.files||[];if(!files.length){box.innerHTML='<b>فایل‌های بیمار:</b><br>فایلی ثبت نشده است.';box.style.display='block';return}box.innerHTML='<b>فایل‌های بیمار:</b><br>'+files.map(function(file){var fn=typeof file==='string'?file:file.filename;var title=typeof file==='string'?'':(file.title||'');return '<a target="_blank" href="/api/files/'+encodeURIComponent(patientCode)+'/'+encodeURIComponent(fn)+'">'+(title||fn)+'</a>'}).join('<br>');box.style.display='block'}catch(e){box.innerHTML='خطا در دریافت فایل‌ها: '+e.message;box.style.display='block'}}})();</script>'''
@app.route('/')
def home():
 ...
 html=html.replace('</body>',PERSIAN_DATE_SCRIPT+UNIFIED_FILES_SCRIPT+MODQ_SCRIPT+PATIENT_LIST_SCRIPT+SAVE_OVERRIDE_SCRIPT+'</body>'); r=Response(html,mimetype='text/html'); r.headers['Cache-Control']='no-store, no-cache, must-revalidate, max-age=0'; return r
@app.route('/modq')
@login_required
def modq_page(): return render_template('modq.html')
@app.route('/api/patients/next-row')
@login_required
def next_patient_row(): return jsonify({'next_row':(db.session.query(db.func.max(Patient.id)).scalar() or 0)+1})
...
def delete_patient(patient_code):
 ...
 try:
  ...
  PatientFile.query.filter_by(patient_code=code).delete(synchronize_session=False); db.session.delete(p); db.session.commit(); return jsonify({'success':True})

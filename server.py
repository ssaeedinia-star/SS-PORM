...
const oldSave=window.savePatient;document.addEventListener('DOMContentLoaded',function(){installRecordNumber();});window.addEventListener('pageshow',refreshRecordNumber);
</script>'''
@app.route('/')
def home():
...
def next_patient_row():
 max_id=db.session.query(db.func.max(Patient.id)).scalar() or 0
 return jsonify({'next_row':max_id+1})

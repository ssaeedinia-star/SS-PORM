...
CLEAR_FORM_SCRIPT=r'''<script>(function(){function clearPatientForm(){var f=document.getElementById('f');if(!f)return;f.reset();var ids=['bmi','mfi','mfic','modqScore','odi'];ids.forEach(function(id){var e=document.getElementById(id);if(e){if(id==='mfi')e.textContent='0.00';else if(id==='mfic')e.textContent='0';else e.textContent='—';}});var ls=document.getElementById('loadStatus');if(ls)ls.textContent='';}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',clearPatientForm);else clearPatientForm();window.addEventListener('pageshow',function(e){if(e.persisted)clearPatientForm();});})();</script>'''

@app.route("/")
def home():
    with open("index.html","r",encoding="utf-8") as f: html=f.read()
    if "persian-datepicker.min.css" not in html: html=html.replace("</head>",PERSIAN_DATE_ASSETS+"</head>")
    html=html.replace("</body>",PERSIAN_DATE_SCRIPT+CLEAR_FORM_SCRIPT+"</body>")
...
@app.route("/upload")
def upload_page():return render_template("upload.html",patient_code=request.args.get("patient",""))

...
def upload_file(patient_code):
    ...
    file_date = file_date.translate(str.maketrans("۰۱۲۳۴۵۶۷۸۹", "0123456789"))
    file_type = request.form.get("type", "").strip()
    file_stage = request.form.get("stage", "").strip()
    object_key = f"{patient_code_safe}/{filename}"

    s3.upload_fileobj(file, LIARA_BUCKET_NAME, object_key, ExtraArgs={
        "ContentType": file.content_type or "application/octet-stream",
        "Metadata": {"title": file_title, "date": file_date, "type": file_type, "stage": file_stage}
    })
